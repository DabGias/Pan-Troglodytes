...
if __name__ == "__main__":
  bot = commands.Bot(command_prefix=["m, ", "macaco, "],
                     intents=Intents.all(),
                     help_command=None)
  ...
  @bot.command(brief="Retorna ping.", description="Retorna o ping do bot.")
  async def ping(ctx):
  ...
  async def bot_help(ctx, comm=None):
    if comm:
      ...
      embed.add_field(name="Comandos: ", value=comm_desc, inline=False)
  ...
  async def user_info(ctx, member: Member = None):
    ...
    embed.add_field(name="Nome: ", value=target.name, inline=False)
    embed.add_field(name="É um bot? ",
    ...
    embed.add_field(
      name="Atividade: ",
      value="{} {}".format(
        str(target.activity.type).split(".")[-1].title(), target.activity.name)
      if target.activity else "{} não está em nenhuma atividade no momento!".
      format(target.nick if target.nick else target.name),
      inline=False)
  ...
  async def server_info(ctx):
    ...
    status = [
      len(list(filter(lambda m: str(m.status) == "online",
                      ctx.guild.members))),
    ...
    embed.add_field(name="Criado em: ",
                    ...
                    inline=False)
    embed.add_field(name="Total de membros: ",
                    value=len(ctx.guild.members),
                    inline=False)
    embed.add_field(name="Total de humanos: ",
                    value=len(
                      list(filter(lambda m: not m.bot, ctx.guild.members))),
                    inline=False)
    embed.add_field(name="Total de bots: ",
    ...
    embed.add_field(name="Status: ",
                  ...
                  :black_circle: {}""".format(status[0], status[1], status[2],
                                              status[3]),
                    inline=False)
  ...
  async def all_members(ctx):
    ...
    embed.add_field(name="",
                    value="O número de membros em {} é de: {}".format(
  ...
  async def search_google(ctx, query, num_links=5):
    ...
    if 20 >= num_links > 0:
      ...
      links = [
        x for x in search(query, safe="on", num=num_links, stop=num_links)
      ]

      for i, link in enumerate(links):
        embed.add_field(name="**{}.** {}".format(i + 1, link),
                        value="",
                        inline=False)
    else:
      ...
      embed.add_field(
        name="",
        value=
        "**:x: O número de links deve ser menor ou igual a 20 e maior que 0!**",
        inline=False)

    await ctx.send(embed=embed)

  @bot.command(brief="Retorna um número aleatório entre dois números.",
               description="""
  ...
  @bot.command(brief="Retorna uma escolha aleatória.",
               description="""
  ...
  async def choose(ctx, *choices):
    ...
    embed.add_field(name="",
                    value="A minha escolha foi: **{}**".format(
